	...
	<table class="table we7-table table-hover vertical-middle" ng-show="show == 'upgrade'" ng-if="isFounder == 1 && upgradeInfo.from == 'cloud' && ((branch.displayorder >= upgradeInfo.site_branch.displayorder && moduleinfo.application_type != 2) || moduleinfo.application_type == 2)" ng-repeat="branch in upgradeInfo.branches">
		...
		<tr>
			...
			<td class="text-right">
				...
				<?php  if(permission_check_account_user('see_module_manage_system_ugrade')) { ?>
				<a href="javascript:;" ng-click="downgrade(upgradeInfo.name)" class="btn btn-danger" ng-if="branch.id == upgradeInfo.site_branch.id && branch.version.version ==  moduleinfo.version && !moduleinfo.service_expire && moduleinfo.is_encryption">降低版本</a>
				<a ng-if="localUpgradeInfo" href="<?php  echo url('module/manage-system/upgrade')?>module_name={{ moduleinfo.name }}" class="btn btn-primary">本地升级</a>
